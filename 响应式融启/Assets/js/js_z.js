...
$(function(){
    ...
    $('.nav .nav_ico').click(function(){
        $(this).siblings('.section-btn').toggleClass('section-btn_a');
    });
    $('.r_navm .n_ico').click(function(){
        $(this).siblings('.n_m').slideToggle();
    });
    //$('#goBack').on('click',function(){
//		window.history.back();
//	});
    var w_w=$('.scd').width();
    $('.scd .scd_r').width(w_w-275+'px');

    $('.scd_r .r_nav .goBack').click(function(){
        $(this).toggleClass('zk');
        $('.scd .scd_l').toggleClass('scd_la');
        $('.scd .scd_r').toggleClass('scd_ra');
        $('.scd_r .r_nav').toggleClass('r_nava');
    });

    $('.r_navm a:first-child').css('border-left',0);

    $(".tabBox .tabCont:first").css("display","block");
    $(".tabBox .tabNav li").click(function(){
        $(this).siblings("li").removeClass("now");
        $(this).addClass("now");
        $(this).parent(".tabNav").parent(".tabBox").children(".tabCont").css("display","none");
        var i=$(this).index();
        $(this).parent(".tabNav").parent(".tabBox").children(".tabCont:eq("+i+")").css("display","block");
    });
    ...
    $('.s_d .s_dl .ls tr:nth-child(2n+1)').children('td').css('background','#f3f3f3');
